...
function Frame(props) {
  ...
  const navigationLinks = [
    { label: 'Beranda', to: '/dashboard', icon: <DashboardIcon /> },
    { label: 'Produk', to: '/product', icon: <ShoppingCartIcon /> },
    { label: 'Pelanggan', to: '/customer', icon: <PersonIcon /> },
    { label: 'Transaksi', to: '/bill', icon: <PointOfSaleIcon /> },
  ]
      ...
      <List>
        {navigationLinks.map((link) => (
          <NavLink key={link.to} to={link.to} style={({ isActive }) => ({
            display: 'block',
            textDecoration: 'none',
            ...(isActive && { backgroundColor: theme.palette.primary.main, color: '#fff', borderRadius: '4px' }),
            ...(!isActive && { color: theme.palette.text.primary }),
          })}>
            <ListItem disablePadding sx={{ display: 'block' }}>
              <ListItemButton sx={{ minHeight: 48, justifyContent: open ? 'initial' : 'center', px: 2.5 }}>
                <ListItemIcon sx={{ minWidth: 0, mr: open ? 3 : 'auto', justifyContent: 'center', color: 'inherit' }}>{link.icon}</ListItemIcon>
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <AppBar position="fixed" open={open}>
        <Toolbar>
          ...
          <Typography variant="h6" noWrap component="div">Aplikasi Laundry</Typography>
          <Box sx={{ flexGrow: 1 }} />
      ...
      {isMobile ? (
        <Drawer container={container} variant="temporary" open={mobileOpen} onClose={handleMobileDrawerToggle} ModalProps={{ keepMounted: true }} sx={{ display: { xs: 'block', sm: 'none' }, '& .MuiDrawer-paper': { boxSizing: 'border-box', width: drawerWidth } }}>{drawerContent}</Drawer>
      ) : (
        <DesktopDrawer variant="permanent" open={open} sx={{ display: { xs: 'none', sm: 'block' } }}>{drawerContent}</DesktopDrawer>
      )}

      <Box component="main" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        <DrawerHeader />
        <Box sx={{ flex: 1, p: 4, pt: 3 }}>{children}</Box>
